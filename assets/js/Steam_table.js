...
async function updateSteamTable(period) {
    try {
        const response = await fetch(`api/get_steam_table.php?period=${period}`);
        if (!response.ok) {
        ...
        data.forEach((row, index) => {
            ...
            html += `
                <tr class="${rowClass} hover:bg-gray-100 transition-colors">
                    ...
                    <td class="px-4 py-2 border border-gray-200 text-right font-bold">${row.Tong_F3.toFixed(1)}</td>
                </tr>
            `;
        });
        ...
        html += `
            <tr class="bg-blue-50 font-bold sticky bottom-0">
                ...
                <td class="px-4 py-2 border border-gray-200 text-right">${parseFloat(averages.avg_Tong_F3).toFixed(1)}</td>
            </tr>
        `;
